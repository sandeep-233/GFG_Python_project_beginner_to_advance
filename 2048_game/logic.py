...
def start_game():
    # declearing an empty list then appending 4 list each with four elemets as 0
    mat = []
    for i in range(4):
        mat.append([0]*4)
...
def add_new_2(mat):
    ...
    # while loop will break as the random cell chosed will be empty (or contains)
    while(mat[r][c] != 0):
        r = random.randint(0, 3)
        c = random.randint(0, 3)
...
def get_current_state(mat):
    ...
    # or if no cell is empty now, but if after any move left, right, up or down if any two cells gets merged and create an empty
...
# function to compress the grid after every step befor and after merging cells.
def compress(mat):
    # bool varibel to determine any change happened or not 
    changed = False
...
# function to update the matrix if we move / swip left
def move_left(grid):
    ...
    # return new matrix and bool changed telling whether the grid is same or differnt 
